...
@Composable
fun HomeBooksList(books: List<Book>, onBookClick: (book: Book) -> Unit) {
    LazyColumn(
        modifier = Modifier.padding(Spacing.spacing16dp)
    ) {
        items(books) { book ->
            HomeBooksListItem(book = book) { book -> onBookClick(book) }
        }
    }
}

@Composable
fun HomeBooksListItem(book: Book, onBookClick: (book: Book) -> Unit) {
    Text(
        text = book.title,
...
fun HomeBooksListPreview() {
    HomeBooksList(books = BookFactory.sampleList(), onBookClick = {})
}